...
const reportConnection = (status, extra = {}) =>
  api('/api/v1/connection', {
    method: 'POST',
    body: JSON.stringify({ status, device_name: DEVICE_NAME, ...extra }),
  }).catch((e) => console.warn('⚠️  reportConnection:', e.message));

const fetchPairRequest = () =>
  api('/api/v1/connection')
    .then((r) => r.pair_request_phone || null)
    .catch(() => null);
...
client.on('qr', async (qr) => {
  lastQr = qr;
  console.log('\n📱 Scan this QR with WhatsApp -> Linked Devices (also shown in your dashboard):\n');
  qrcode.generate(qr, { small: true });
  // Push the raw QR string to the dashboard so it can be scanned there instead of in the logs.
  await reportConnection('connecting', { qr_code: qr });

  // Did the user ask for a phone-number pairing code instead of scanning?
  if (pairingRequested) return;
  const phone = await fetchPairRequest();
  if (!phone) return;
  ...
  try {
    ...
    await reportConnection('connecting', { pairing_code: code, clear_pair_request: true });
  } catch (e) {
    pairingRequested = false;
    console.warn('⚠️  requestPairingCode:', e.message);
    await reportConnection('connecting', { clear_pair_request: true });
  }
});
...
client.on('ready', async () => {
  ...
  lastQr = null;
  pairingRequested = false;
  await reportConnection('connected', phone ? { phone_number: phone } : {});
...
// While unlinked, watch for a phone-number pairing request coming from the dashboard.
setInterval(async () => {
  if (client.info || pairingRequested || !lastQr) return;
  const phone = await fetchPairRequest();
  if (!phone) return;
  pairingRequested = true;
  try {
